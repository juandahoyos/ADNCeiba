...
public class ControlParqueaderoServiceImp implements IControlParqueaderoService {
	...
	public Parqueadero registroVehiculo(Vehiculo vehiculo) throws ParqueaderoErrorBuilderException {
		if (!restriccionPlaca.validadSiEsDomingoOLunes()
				&& controlParqueadero.validarPlacaIniciaPorLetraA(vehiculo.getPlaca())) {
			throw new ParqueaderoErrorBuilderException(CatalogoMensajes.PLACA_INVALIDA_PARA_INGRESO,
					HttpStatus.NOT_ACCEPTABLE);
		}
		if (!controlParqueadero.buscarEspacioPorTipoVehiculo(vehiculo.getTipoVehiculo())) {
			throw new ParqueaderoErrorBuilderException(CatalogoMensajes.NO_HAY_ESPACIO_PARA_EL_TIPO_DE_VEHICULO,
					HttpStatus.NOT_ACCEPTABLE);
		}
		if (controlParqueadero.buscarVehiculoEstacionado(vehiculo.getPlaca())) {
			throw new ParqueaderoErrorBuilderException(CatalogoMensajes.VEHICULO_YA_SE_ENCUENTRA_ESTACIONADO,
					HttpStatus.NOT_ACCEPTABLE);
		}
	...
	public Parqueadero salidaVehiculo(String placa) throws ParqueaderoErrorBuilderException {
		...
		if (!controlParqueadero.buscarVehiculoEstacionado(placa)) {
			throw new ParqueaderoErrorBuilderException(CatalogoMensajes.VEHICULO_NO_ESTA_ESTACIONADO,
					HttpStatus.NOT_ACCEPTABLE);
		}
		fechaSalida = Calendar.getInstance().getTime();
		parqueadero = parqueaderoService.getParqueaderoParaAsignar(placa);
		cobro = controlParqueadero.generarCobro(parqueadero.getVehiculo().getTipoVehiculo(),
				parqueadero.getFechaIngreso(), fechaSalida, parqueadero.getVehiculo().getCilindraje());
	...
	public List<Parqueadero> consultarTodosLosVehiculos() throws ParqueaderoErrorBuilderException {
		if (controlParqueadero.validarSiPaqueaderoEstaVacio()) {
			throw new ParqueaderoErrorBuilderException(CatalogoMensajes.PARQUEADERO_ESTA_VACIO,
					HttpStatus.NOT_ACCEPTABLE);
		}
